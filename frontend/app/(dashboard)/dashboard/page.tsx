...
import { FolderKanban, Users, CheckCircle, Clock, Loader2, ArrowUpRight, Building2 } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const { data: projects, isLoading: projectsLoading } = useQuery({
    ...
    enabled: hasPermission("user:read") && !isSuperAdmin,
  });
  ...
  const isLoading = isSuperAdmin ? companiesLoading : (projectsLoading || usersLoading);
      ...
      <motion.div variants={itemAnim} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-4">
        <div>
          ...
          <p className="text-zinc-500 dark:text-zinc-400 mt-2 font-medium">
            {isSuperAdmin ? "Here is a quick overview of NexusAI platforms" : "Here is a quick overview of your NexusAI workspace"}
          </p>
        </div>
      </motion.div>
