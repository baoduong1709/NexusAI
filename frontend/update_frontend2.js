...
const newSendCode = `  const chatMutation = { isPending: false }; // DUMMY TO NOT BREAK CSS CLASS
...
// Find `const chatMutation = useMutation(` to `});`
const chatMutationRegex = /  const chatMutation = useMutation\(\{[\s\S]*?toast\.error\(e\.response\?\.data\?\.message \|\| "Chat failed"\),\n  \}\);/;
code = code.replace(chatMutationRegex, '');

const sendRegex = /  const send = \(\) => \{[\s\S]*?chatMutation\.mutate\(\{ userMsg: msg, currentMessages: updatedMessages \}\);\n  \};/;
code = code.replace(sendRegex, newSendCode);

// Fix chatMutation.isPending references
code = code.replace(/chatMutation\.isPending/g, 'isStreaming');

fs.writeFileSync(path, code);
console.log("Updated ai-chat-bubble.tsx cleanly");
